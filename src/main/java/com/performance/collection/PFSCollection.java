 ...
 */
@Controller
public class PFSCollection extends HttpServlet {
	...
	@RequestMapping(value = "collectPFS")
	public void CollectPFS(HttpServletRequest request,
		...
		if (!startTime.isEmpty() && !endTime.isEmpty()) {
			...
			try {
				...
				devDe.Connect("10.65.193.25:1521", "orayali2", "qa", "qa");
				String sql = "select to_CHAR(gmt_create, 'yyyy-mm-dd hh24:mi:ss') as orderDate,count(1) as orderCount from payment.TB_BIZ_PAYMENT_ORDER t where t.GMT_CREATE >= to_date('"
						+ formatStartTime
			...
			try {
				DbEnviornment devDe = new DbEnviornment();
				devDe.Connect("10.65.193.25:1521", "orayali2", "qa", "qa");
				String sql = "select to_CHAR(gmt_create, 'yyyy-mm-dd hh24:mi:ss') as orderDate,count(1) as orderCount from payment.TB_BIZ_PAYMENT_ORDER t where t.GMT_CREATE >= sysdate - 1/24/60 and t.GMT_CREATE <= sysdate and t.BIZ_PRODUCT_CODE = '"
						+ bizCode
						+ "' GROUP BY to_CHAR(gmt_create, 'yyyy-mm-dd hh24:mi:ss') ORDER BY to_CHAR(gmt_create, 'yyyy-mm-dd hh24:mi:ss') ASC";
				ResultSet queryResult = DbEnviornment.statement
						.executeQuery(sql);
