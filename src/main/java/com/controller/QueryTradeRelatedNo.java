...
import com.mgs.bean.QueryTradeRelatedNoBean;
 ...
 */
@Controller
public class QueryTradeRelatedNo extends HttpServlet {
	...
	@RequestMapping(value = "QueryTradeRelatedNo")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("qtrnBean") QueryTradeRelatedNoBean qtrnBean,
		...
		if (!qtrnBean.getTrade_related_no().isEmpty()) {
			inputString.append(",trade_related_no=" + qtrnBean.getTrade_related_no());
		}
		if (!qtrnBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qtrnBean.getExtend_param());
		}
		...
		qtrnBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			qtrnBean.setEncodeString(tool.removeFromString(
					qtrnBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qtrnBean.getEncodeString() + "&sign="
