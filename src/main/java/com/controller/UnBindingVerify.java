...
import com.mgs.bean.UnBindingVerifyBean;
 ...
 */
@Controller
public class UnBindingVerify extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("ubvBean") UnBindingVerifyBean ubvBean,
		...
		if (!ubvBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + ubvBean.getIdentity_id());
		}
		if (!ubvBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + ubvBean.getIdentity_type());
		}
		if (!ubvBean.getVerify_type().isEmpty()) {
			inputString.append(",verify_type=" + ubvBean.getVerify_type());
		}
		if (!ubvBean.getClient_ip().isEmpty() && ubvBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + ubvBean.getClient_ip().replace(",", "#"));
		}else if (!ubvBean.getClient_ip().isEmpty() && !ubvBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + ubvBean.getClient_ip());
		}
		if (!ubvBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + ubvBean.getExtend_param());
		}
		...
		ubvBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			ubvBean.setEncodeString(tool.removeFromString(
					ubvBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = ubvBean.getEncodeString() + "&sign="
