...
import com.mgs.bean.UnBindingBankCardAdvanceBean;
 ...
 */
@Controller
public class UnBindingBankCardAdvance extends HttpServlet {
	...
	@RequestMapping(value = "UnBindingBankCardAdvance")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("ubbcBean") UnBindingBankCardAdvanceBean ubbcaBean,
		...
		if (!ubbcaBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + ubbcaBean.getIdentity_id());
		}
		if (!ubbcaBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + ubbcaBean.getIdentity_type());
		}
		if (!ubbcaBean.getTicket().isEmpty()) {
			inputString.append(",ticket=" + ubbcaBean.getTicket());
		}
		if (!ubbcaBean.getValid_code().isEmpty()) {
			inputString.append(",valid_code=" + ubbcaBean.getValid_code());
		}
		if (!ubbcaBean.getClient_ip().isEmpty() && ubbcaBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + ubbcaBean.getClient_ip().replace(",", "#"));
		}else if (!ubbcaBean.getClient_ip().isEmpty() && !ubbcaBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + ubbcaBean.getClient_ip());
		}
		if (!ubbcaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + ubbcaBean.getExtend_param());
		}
		...
		ubbcaBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			ubbcaBean.setEncodeString(tool.removeFromString(
					ubbcaBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = ubbcaBean.getEncodeString() + "&sign="
