...
import com.mgs.bean.PageBindingBankCardBean;
 ...
 */
@Controller
public class PageBindingBankCard extends HttpServlet {
	...
	@RequestMapping(value = "PageBindingBankCard")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("bbcBean") PageBindingBankCardBean bbcBean,
		...
		if (!bbcBean.getReal_neme().isEmpty()) {
			try {
				encryptRealName = rsa.encrypt(bbcBean.getReal_neme(),
		...
		if (!bbcBean.getWithhold_param().isEmpty() && bbcBean.getWithhold_param().contains(",")) {
			inputString.append(",withhold_param=" + bbcBean.getWithhold_param().replace(",", "#"));
		}else if (!bbcBean.getWithhold_param().isEmpty() && !bbcBean.getWithhold_param().contains(",")){
			inputString.append(",withhold_param=" + bbcBean.getWithhold_param());
		}
		...
		if (!bbcBean.getWithhold_param().isEmpty()) {
			bbcBean.setEncodeString(tool.removeFromString(
					bbcBean.getEncodeString(), "&", "&", "startswith",
					"withhold_param")
					+ "&withhold_param="
					+ tool.textEncode(bbcBean.getWithhold_param(), "UTF-8"));
		}
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			bbcBean.setEncodeString(tool.removeFromString(
					bbcBean.getEncodeString(), "&", "&", "startswith",
