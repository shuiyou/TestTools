 ...
 */
@Controller
public class PayHostingTrade extends HttpServlet {
	...
	@RequestMapping(value = "PayHostingTrade")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("phtBean") PayHostingTradeBean phtBean,
		...
		if (!phtBean.getOut_pay_no().isEmpty()) {
			inputString.append(",out_pay_no=" + phtBean.getOut_pay_no());
		}
		if (!phtBean.getOuter_trade_no_list().isEmpty()) {
			inputString.append(",outer_trade_no_list="
					+ phtBean.getOuter_trade_no_list());
		}
