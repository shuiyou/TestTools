...
import com.b2c.bean.CreateB2CRefundBean;
 ...
 */
@Controller
public class CreateB2CRefund extends HttpServlet {
	...
	@RequestMapping(value = "CreateB2CRefund")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("crBean") CreateB2CRefundBean crBean,
		...
		if (!crBean.getOut_trade_no().isEmpty()) {
			inputString.append(",out_trade_no=" + crBean.getOut_trade_no());
		}
		if (!crBean.getRefund_amount().isEmpty()) {
			inputString.append(",refund_amount=" + crBean.getRefund_amount());
		}
		if (!crBean.getOrig_out_trade_no().isEmpty()) {
			inputString.append(",orig_out_trade_no=" + crBean.getOrig_out_trade_no());
		}
		if (!crBean.getSummary().isEmpty()) {
			inputString.append(",summary=" + crBean.getSummary());
		}
		if (!crBean.getSplit_list().isEmpty()) {
			inputString.append(",split_list=" + crBean.getSplit_list());
		}
		if (!crBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + crBean.getExtend_param());
		}
		...
		crBean.setEncodeString(sortString);
		if (!b2cBaseBean.getNotify_url().isEmpty()) {
			crBean.setEncodeString(tool.removeFromString(
					crBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		if (!crBean.getSummary().isEmpty()) {
			crBean.setEncodeString(tool.removeFromString(
					...
					"summary")
					+ "&summary="
					+ tool.textEncode(crBean.getSummary(), "UTF-8"));
		}
		if (!crBean.getSplit_list().isEmpty()) {
			crBean.setEncodeString(tool.removeFromString(
					crBean.getEncodeString(), "&", "&", "startswith",
					"split_list")
					+ "&split_list="
					+ tool.textEncode(crBean.getSplit_list(), "UTF-8"));
		}
		if (!crBean.getExtend_param().isEmpty()) {
			crBean.setEncodeString(tool.removeFromString(
					crBean.getEncodeString(), "&", "&", "startswith",
					"extend_param")
					+ "&extend_param="
					+ tool.textEncode(crBean.getExtend_param(), "UTF-8"));
		}

		String requestString = crBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
