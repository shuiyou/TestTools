...
import com.mgs.bean.BindingVerifyBean;
 ...
 */
@Controller
public class BindingVerify extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("bvBean") BindingVerifyBean bvBean,
		...
		String encryptVerifyEntity = null;
		...
		if (!bvBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + bvBean.getIdentity_id());
		}
		if (!bvBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + bvBean.getIdentity_type());
		}
		if (!bvBean.getVerify_type().isEmpty()) {
			inputString.append(",verify_type=" + bvBean.getVerify_type());
		}
		if (!bvBean.getVerify_entity().isEmpty()) {
			try {
				encryptVerifyEntity = rsa.encrypt(bvBean.getVerify_entity(),
						mgsBaseBean.getEncryptkey());
				inputString.append(",verify_entity=" + encryptVerifyEntity);
			} catch (Exception e) {
		...
		if (!bvBean.getClient_ip().isEmpty() && bvBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + bvBean.getClient_ip().replace(",", "#"));
		}else if (!bvBean.getClient_ip().isEmpty() && !bvBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + bvBean.getClient_ip());
		}

		if (!bvBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + bvBean.getExtend_param());
		}
		...
		bvBean.setEncodeString(sortString);
		if (encryptVerifyEntity != null) {
			bvBean.setEncodeString(tool.removeFromString(
					bvBean.getEncodeString(), "&", "&", "startswith",
					"verify_entity")
					+ "&verify_entity="
					+ tool.textEncode(encryptVerifyEntity, "UTF-8"));
		}
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			bvBean.setEncodeString(tool.removeFromString(
					bvBean.getEncodeString(), "&", "&", "startswith",
		...
		String requestString = bvBean.getEncodeString() + "&sign="
