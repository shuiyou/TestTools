...
import com.pas.bean.ValidateIdentityBean;
 ...
 */
@Controller
public class ValidateIdentity extends HttpServlet {
	...
	public void doPost(@ModelAttribute("PasBaseBean") PasBaseBean pasBaseBean,
			@ModelAttribute("viBean") ValidateIdentityBean viBean,
		...
		if (!viBean.getOut_request_no().isEmpty()) {
			inputString.append(",out_request_no=" + viBean.getOut_request_no());
		}
		if (!viBean.getCert_type().isEmpty()) {
			inputString.append(",cert_type=" + viBean.getCert_type());
		}
		if (!viBean.getExtend_param().isEmpty()) {
			inputString.append(",cert_type=" + viBean.getExtend_param());
		}
        String encryptRealName = null;
		if (!viBean.getReal_name().isEmpty()) {
			try {
				encryptRealName = rsa.encrypt(viBean.getReal_name(),
		...
		if (!viBean.getCert_no().isEmpty()) {
			try {
				encryptCertNo = rsa.encrypt(viBean.getCert_no(),
						pasBaseBean.getEncryptkey());
		...
		viBean.setEncodeString(sortString);
		if (encryptRealName != null) {
			viBean.setEncodeString(tool.removeFromString(
					viBean.getEncodeString(), "&", "&", "startswith",
					"real_name")
					+ "&real_name="
		...
		String requestString = viBean.getEncodeString() + "&sign="
