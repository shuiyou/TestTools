 ...
 */
@Controller
public class ValidatePhoneNo extends HttpServlet {
    ...
    @RequestMapping(value = "ValidatePhoneNo")
    public void doPost(@ModelAttribute("PasBaseBean") PasBaseBean pasBaseBean,
                       @ModelAttribute("vpnBean") ValidatePhoneNoBean vpnBean,
        ...
        if (!vpnBean.getPhone_no().isEmpty()) {
            try {
            	encryptPhoneNo = rsa.encrypt(vpnBean.getPhone_no(),
        ...
        if (!vpnBean.getExtend_param().isEmpty()) {
            inputString.append(",cert_type=" + vpnBean.getExtend_param());
        }
