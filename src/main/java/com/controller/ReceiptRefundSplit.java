...
import com.b2c.bean.ReceiptRefundSplitBean;
...
@Controller
public class ReceiptRefundSplit extends HttpServlet {
 ...
 */
	...
	@RequestMapping(value = "ReceiptRefundSplit")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("cboBean") ReceiptRefundSplitBean cboBean,
		...
		if (!cboBean.getUser_ip().isEmpty()) {
			inputString.append(",user_ip=" + cboBean.getUser_ip());
		}
		if (!cboBean.getRefund_split_list().isEmpty()) {
			inputString.append(",refund_split_list="
					+ cboBean.getRefund_split_list());
		}
		...
		if (!cboBean.getRefund_split_list().isEmpty()) {
			cboBean.setEncodeString(tool.removeFromString(
					...
					"refund_split_list")
					+ "&refund_split_list="
					+ tool.textEncode(cboBean.getRefund_split_list(), "UTF-8"));
		}
