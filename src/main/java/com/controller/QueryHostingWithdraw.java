...
import com.mas.bean.QueryHostingWithdrawBean;
 ...
 */
@Controller
public class QueryHostingWithdraw extends HttpServlet {
	...
	@RequestMapping(value = "QueryHostingWithdraw")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("qhwBean") QueryHostingWithdrawBean qhwBean,
		...
		if (!qhwBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + qhwBean.getIdentity_id());
		}
		if (!qhwBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + qhwBean.getIdentity_type());
		}
		if (!qhwBean.getAccount_type().isEmpty()) {
			inputString.append(",account_type=" + qhwBean.getAccount_type());
		}
		if (!qhwBean.getOut_trade_no().isEmpty()) {
			inputString.append(",out_trade_no=" + qhwBean.getOut_trade_no());
		}
		if (!qhwBean.getStart_time().isEmpty()) {
			inputString.append(",start_time=" + qhwBean.getStart_time());
		}
		if (!qhwBean.getEnd_time().isEmpty()) {
			inputString.append(",end_time=" + qhwBean.getEnd_time());
		}
		if (!qhwBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qhwBean.getExtend_param());
		}
		if (!qhwBean.getPage_no().isEmpty()) {
			inputString.append(",page_no=" + qhwBean.getPage_no());
		}
		if (!qhwBean.getPage_size().isEmpty()) {
			inputString.append(",page_size=" + qhwBean.getPage_size());
		}
		...
		qhwBean.setEncodeString(sortString);
		if (!masBaseBean.getNotify_url().isEmpty()) {
			qhwBean.setEncodeString(tool.removeFromString(
					qhwBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qhwBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
