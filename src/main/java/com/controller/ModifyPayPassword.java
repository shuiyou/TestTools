...
import com.mgs.bean.ModifyPayPasswordBean;
 ...
 */
@Controller
public class ModifyPayPassword extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("camBean") ModifyPayPasswordBean mppBean,
		...
		if (!mppBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + mppBean.getIdentity_id());
		}
		if (!mppBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + mppBean.getIdentity_type());
		}
		...
		if (!mppBean.getClient_ip().isEmpty() && mppBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + mppBean.getClient_ip().replace(",", "#"));
		}else if (!mppBean.getClient_ip().isEmpty() && !mppBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + mppBean.getClient_ip());
		}
		if (!mppBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + mppBean.getExtend_param());
		}
		...
		mppBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			mppBean.setEncodeString(tool.removeFromString(
					mppBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = mppBean.getEncodeString() + "&sign="
