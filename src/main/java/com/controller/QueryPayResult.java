...
import com.mas.bean.QueryPayResultBean;
 ...
 */
@Controller
public class QueryPayResult extends HttpServlet {
	...
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("qprBeean") QueryPayResultBean qprBeean,
		...
		if (!qprBeean.getOut_pay_no().isEmpty()) {
			inputString.append(",out_pay_no=" + qprBeean.getOut_pay_no());
		}
		if (!qprBeean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qprBeean.getExtend_param());
		}
		...
		qprBeean.setEncodeString(sortString);
		if (!masBaseBean.getNotify_url().isEmpty()) {
			qprBeean.setEncodeString(tool.removeFromString(
					qprBeean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qprBeean.getEncodeString().replaceAll(
				"replaceFlag", ",")
