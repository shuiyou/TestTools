...
import com.mgs.bean.CreateMemberAccountBean;
 ...
 */
@Controller
public class CreateMemberAccount extends HttpServlet {
	...
	@RequestMapping(value = "CreateMemberAccount")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("camBean") CreateMemberAccountBean sppBean,
		...
		if (!sppBean.getMember_type().isEmpty()) {
			inputString.append(",member_type=" + sppBean.getMember_type());
		}
		if (!sppBean.getAccount_type().isEmpty()) {
			inputString.append(",account_type=" + sppBean.getAccount_type());
		}
