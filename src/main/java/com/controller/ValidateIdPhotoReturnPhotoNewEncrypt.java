...
import com.pas.bean.ValidateIdPhotoReturnPhotoNewEncryptBean;
 ...
 */
@Controller
public class ValidateIdPhotoReturnPhotoNewEncrypt extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "ValidateIdPhotoReturnPhotoNewEncrypt")
	public void doPost(@ModelAttribute("pasBaseBean") PasBaseBean pasBaseBean,
			@ModelAttribute("viprpneBean") ValidateIdPhotoReturnPhotoNewEncryptBean viprpneBean,
			HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		if (!viprpneBean.getOut_request_no().isEmpty()) {
			formparams.add(new OrderBasicNameValuePair("out_request_no", viprpneBean.getOut_request_no()));
		}

		if (!viprpneBean.getExtend_param().isEmpty()) {
			formparams.add(new OrderBasicNameValuePair("extend_param" , viprpneBean.getExtend_param()));
		}
		if(!viprpneBean.getCert_no().isEmpty()){
			String certnoEncrypt = rsa.encrypt(viprpneBean.getCert_no(),pasBaseBean.getEncryptkey());
			formparams.add( new OrderBasicNameValuePair( "cert_no",certnoEncrypt));
		}
		if(!viprpneBean.getCert_type().isEmpty()){
			formparams.add( new OrderBasicNameValuePair( "cert_type",viprpneBean.getCert_type()));
		}
		if(!viprpneBean.getReal_name().isEmpty()){
			String realnameEncrypt = rsa.encrypt( viprpneBean.getReal_name(),pasBaseBean.getEncryptkey());
			formparams.add( new OrderBasicNameValuePair( "real_name",realnameEncrypt));
		}

		//if (!vbcBean.getPic_file().isEmpty()) {
		formparams.add(new OrderBasicNameValuePair("pic_file", viprpneBean.getPic_file()));

		formparams.add(new OrderBasicNameValuePair("pic_file2", viprpneBean.getPic_file2()));
		//}
