...
import com.sh_bank.bean.QueryMemberinfos_shBean;

/**
 * Created by Luohaiyang on 2017/7/27.
 */
@Controller
public class QueryMemberInfos_sh extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "QueryMemberInfos_sh")
    public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
                       @ModelAttribute("chrBean") QueryMemberinfos_shBean chrBean, HttpServletRequest request,
        ...
        if (!chrBean.getQuery_card_flag().isEmpty()) {
            inputString.append(",query_card_flag=" + chrBean.getQuery_card_flag());
        }
        if (!chrBean.getQuery_paypwd_set_flag().isEmpty()) {
            inputString.append(",query_paypwd_set_flag=" + chrBean.getQuery_paypwd_set_flag());
        }
