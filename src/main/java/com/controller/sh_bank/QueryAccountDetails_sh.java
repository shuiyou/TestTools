...
import com.sh_bank.bean.QueryAccountDetails_shBean;
 ...
 */
@Controller
public class QueryAccountDetails_sh extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "QueryAccountDetails_sh")
    public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
                       @ModelAttribute("chrBean") QueryAccountDetails_shBean chrBean, HttpServletRequest request,
        ...
        if (!chrBean.getStart_time().isEmpty()) {
            inputString.append(",start_time=" + chrBean.getStart_time());
        }
        if (!chrBean.getEnd_time().isEmpty()) {
            inputString.append(",end_time=" + chrBean.getEnd_time());
        }
        if (!chrBean.getPage_no().isEmpty()) {
            inputString.append(",page_no=" + chrBean.getPage_no());
        }
        if (!chrBean.getPage_size().isEmpty()) {
            inputString.append(",page_size=" + chrBean.getPage_size());
        }
