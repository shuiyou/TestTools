...
import com.sh_bank.bean.AccountFundTransfer_shBean;
 ...
 */
@Controller
public class AccountFundTransfer_sh extends HttpServlet {
	...
	@RequestMapping(value = "AccountFundTransfer_sh")
	public void doPost(@ModelAttribute("masBaseBean") ShMasBaseBean masBaseBean,
			@ModelAttribute("chrBean") AccountFundTransfer_shBean chrBean, HttpServletRequest request,
		...
		if (!chrBean.getPayer_identity_id().isEmpty()) {
			inputString.append(",payer_identity_id=" + chrBean.getPayer_identity_id());
		}
		if (!chrBean.getPayer_identity_type().isEmpty()) {
			inputString.append(",payer_identity_type=" + chrBean.getPayer_identity_type());
		}
		if (!chrBean.getPayer_account_type().isEmpty()) {
			inputString.append(",payer_account_type=" + chrBean.getPayer_account_type());
		}
		if (!chrBean.getPayee_identity_id().isEmpty()) {
			inputString.append(",payee_identity_id=" + chrBean.getPayee_identity_id());
		}
		if (!chrBean.getPayee_identity_type().isEmpty()) {
			inputString.append(",payee_identity_type=" + chrBean.getPayee_identity_type());
		}
		if (!chrBean.getPayee_account_type().isEmpty()) {
			inputString.append(",payee_account_type=" + chrBean.getPayee_account_type());
		}
