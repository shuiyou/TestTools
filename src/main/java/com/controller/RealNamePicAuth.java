...
import com.mgs.bean.RealNamePicAuthBean;
 ...
 */
@Controller
public class RealNamePicAuth extends HttpServlet {
	...
	@RequestMapping(value = "RealNamePicAuth")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("rnpaBean") RealNamePicAuthBean qwaBean,
		...
		if (!qwaBean.getPic_file_list().isEmpty() ) {
			inputString.append(",pic_file_list=" + qwaBean.getPic_file_list());
		}
		if (!qwaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qwaBean.getExtend_param());
		}
		
		if (!qwaBean.getClient_ip().isEmpty() ) {
			inputString.append(",client_ip=" + qwaBean.getClient_ip());
		}
		...
		if (!qwaBean.getPic_file_list().isEmpty()) {
			qwaBean.setEncodeString(tool.removeFromString(
					...
					"pic_file_list")
					+ "&pic_file_list="
					+ tool.textEncode(qwaBean.getPic_file_list(), "UTF-8"));
		}
