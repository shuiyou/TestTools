...
import com.mgs.bean.QueryVerifyBean;
 ...
 */
@Controller
public class QueryVerify extends HttpServlet {
	...
	@RequestMapping(value = "QueryVerify")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("qvBean") QueryVerifyBean qvBean,
		...
		if (!qvBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + qvBean.getIdentity_id());
		}
		if (!qvBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + qvBean.getIdentity_type());
		}
		if (!qvBean.getVerify_type().isEmpty()) {
			inputString.append(",verify_type=" + qvBean.getVerify_type());
		}
		if (!qvBean.getIs_mask().isEmpty()) {
			inputString.append(",is_mask=" + qvBean.getIs_mask());
		}
