...
import com.fund.bean.FundMemberAccountGetBean;
 ...
 */
@Controller
public class FundMemberAccountGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberAccountGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmagBean") FundMemberAccountGetBean fmagBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmagBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmagBean.getMember_id());
        }
        if (!fmagBean.getLogin_name().isEmpty()) {
            inputString.append(",login_name=" + fmagBean.getLogin_name());
        }
        if (!fmagBean.getLogin_name_type().isEmpty()) {
            inputString.append(",login_name_type=" + fmagBean.getLogin_name_type());
        }
        ...
        fmagBean.setEncodeString(sortString);


        String requestString = fmagBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
