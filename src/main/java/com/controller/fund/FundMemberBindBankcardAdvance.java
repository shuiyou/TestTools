...
import com.fund.bean.FundMemberBindBankcardAdvanceBean;
 ...
 */
@Controller
public class FundMemberBindBankcardAdvance extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberBindBankcardAdvance")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmbbadvBean") FundMemberBindBankcardAdvanceBean fmbbadvBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmbbadvBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmbbadvBean.getMember_id());
        }
        if (!fmbbadvBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + fmbbadvBean.getPayment_method_id());
        }
        if (!fmbbadvBean.getValid_code().isEmpty()) {
            inputString.append(",valid_code=" + fmbbadvBean.getValid_code());
        }
        ...
        fmbbadvBean.setEncodeString(sortString);

        String requestString = fmbbadvBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
