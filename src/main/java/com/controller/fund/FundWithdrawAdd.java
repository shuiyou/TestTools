...
import com.fund.bean.FundWithdrawAddBean;
 ...
 */
@Controller
public class FundWithdrawAdd extends HttpServlet {
    ...
    @RequestMapping(value = "FundWithdrawAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fwaBean") FundWithdrawAddBean fwaBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fwaBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fwaBean.getMember_id());
        }
        if (!fwaBean.getAmount().isEmpty()) {
            inputString.append(",amount=" + fwaBean.getAmount());
        }
        if (!fwaBean.getArrival_type().isEmpty()) {
            inputString.append(",arrival_type=" + fwaBean.getArrival_type());
        }
        if (!fwaBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + fwaBean.getOut_request_no());
        }
        ...
        fwaBean.setEncodeString(sortString);

        String requestString = fwaBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
