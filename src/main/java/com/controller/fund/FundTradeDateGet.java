...
import com.fund.bean.FundTradeDateGetBean;
 ...
 */
@Controller
public class FundTradeDateGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundTradeDateGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("ftdgBean") FundTradeDateGetBean ftdgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!ftdgBean.getDate_time().isEmpty()) {
            inputString.append(",date_time=" + ftdgBean.getDate_time());
        }
        if (!ftdgBean.getOffset().isEmpty()) {
            inputString.append(",offset=" + ftdgBean.getOffset());
        }
        ...
        ftdgBean.setEncodeString(sortString);

        String requestString = ftdgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
