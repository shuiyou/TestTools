...
import com.fund.bean.FundMemberIdentityUpdateBean;
 ...
 */
@Controller
public class FundMemberIdentityUpdate extends HttpServlet {

    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundMemberIdentityUpdate")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmiuBean") FundMemberIdentityUpdateBean fmiuBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmiuBean.getLogin_name().isEmpty()) {
            inputString.append(",login_name=" + fmiuBean.getLogin_name());
        }
        if (!fmiuBean.getLogin_name_type().isEmpty()) {
            inputString.append(",login_name_type=" + fmiuBean.getLogin_name_type());
        }
        ...
        /*if (!fmiuBean.getMember_id().isEmpty()) {
            fmiuBean.setEncodeString(tool.removeFromString(fmiuBean.getEncodeString(), "&", "&", "startswith", "member_id") + "&member_id=" + tool.textEncode(memberid, "UTF-8"));
        }
        if (!fmiuBean.getLogin_name().isEmpty()) {
            fmiuBean.setEncodeString(tool.removeFromString(fmiuBean.getEncodeString(), "&", "&", "startswith", "login_name") + "&login_name=" + tool.textEncode(loginname, "UTF-8"));
        }
        if (!fmiuBean.getLogin_name_type().isEmpty()) {
            fmiuBean.setEncodeString(tool.removeFromString(fmiuBean.getEncodeString(), "&", "&", "startswith", "login_name_type") + "&login_name_type=" + tool.textEncode(loginnametype, "UTF-8"));
        }*/

        String requestString = fmiuBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
