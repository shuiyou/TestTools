...
import com.fund.bean.FundRateAndDiscountGetBean;
import com.fund.bean.FundRiskAnswerSurveyAddBean;
 ...
 */
@Controller
public class FundRateAndDiscountGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundRateAndDiscountGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fradgBean") FundRateAndDiscountGetBean frasaBean,
        ...
        if (!frasaBean.getFund_code().isEmpty()) {
            inputString.append(";fund_code=" + frasaBean.getFund_code());
        }
        if (!frasaBean.getBusin_code().isEmpty()) {
            inputString.append(";busin_code=" + frasaBean.getBusin_code());
        }
        if (!frasaBean.getCust_type().isEmpty()){
            inputString.append(";cust_type=" + frasaBean.getCust_type());
        }
        if (!frasaBean.getOther_fund_code().isEmpty()){
            inputString.append(";other_fund_code=" + frasaBean.getOther_fund_code());
        }
        ...
        frasaBean.setEncodeString(sortString);
        String requestString = frasaBean.getEncodeString() + "&sign="
