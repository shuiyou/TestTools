...
import com.fund.bean.FundDividendMethodOrderAddBean;
 ...
 */
@Controller
public class FundDividendMethodOrderAdd extends HttpServlet{
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundDividendMethodOrderAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fdmoaBean") FundDividendMethodOrderAddBean fdmoaBean,
        ...
        if (!fdmoaBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fdmoaBean.getMember_id());
        }
        if (!fdmoaBean.getTrade_account_no().isEmpty()) {
            inputString.append(",trade_account_no=" + fdmoaBean.getTrade_account_no());
        }
        if (!fdmoaBean.getFund_code().isEmpty()) {
            inputString.append(",fund_code=" + fdmoaBean.getFund_code());
        }
        if (!fdmoaBean.getDividend_dethod().isEmpty()) {
            inputString.append(",dividend_dethod=" + fdmoaBean.getDividend_dethod());
        }
        if (!fdmoaBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + fdmoaBean.getOut_request_no());
        }
        ...
        fdmoaBean.setEncodeString(sortString);
        String requestString = fdmoaBean.getEncodeString() + "&sign="
