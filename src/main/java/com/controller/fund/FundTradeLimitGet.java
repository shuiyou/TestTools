...
import com.fund.bean.FundTradeLimitGetBean;
 ...
 */
@Controller
public class FundTradeLimitGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundTradeLimitGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("ftlgBean") FundTradeLimitGetBean ftlgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!ftlgBean.getFund_code().isEmpty()) {
            inputString.append(",fund_code=" + ftlgBean.getFund_code());
        }
        if (!ftlgBean.getBusin_code().isEmpty()) {
            inputString.append(",busin_code=" + ftlgBean.getBusin_code());
        }
        if (!ftlgBean.getCust_type().isEmpty()) {
            inputString.append(",cust_type=" + ftlgBean.getCust_type());
        }
        if (!ftlgBean.getTrust_type().isEmpty()) {
            inputString.append(",trust_type=" + ftlgBean.getTrust_type());
        }
        if (!ftlgBean.getOther_fund_code().isEmpty()) {
            inputString.append(",other_fund_code=" + ftlgBean.getOther_fund_code());
        }
        ...
        ftlgBean.setEncodeString(sortString);

        String requestString = ftlgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
