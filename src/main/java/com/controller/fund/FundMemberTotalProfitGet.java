 ...
 */
@Controller
public class FundMemberTotalProfitGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberTotalProfitGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmtpBean") FundMemberTotalProfitGetBean fmtpBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmtpBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmtpBean.getMember_id());
        }
        if (!fmtpBean.getFund_code_list().isEmpty()) {
            inputString.append(",fund_code_list=" + fmtpBean.getFund_code_list());
        }
        if (!fmtpBean.getFilter_fund_code_list().isEmpty()) {
            inputString.append(",filter_fund_code_list=" + fmtpBean.getFilter_fund_code_list());
        }
        ...
        fmtpBean.setEncodeString(sortString);


        String requestString = fmtpBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
