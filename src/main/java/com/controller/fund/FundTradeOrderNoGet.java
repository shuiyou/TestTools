...
import com.fund.bean.FundTradeOrderNoGetBean;
 ...
 */
@Controller
public class FundTradeOrderNoGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundTradeOrderNoGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmcbvBean") FundTradeOrderNoGetBean ftongBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!ftongBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + ftongBean.getOut_request_no());
        }
        ...
        ftongBean.setEncodeString(sortString);
        String requestString = ftongBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
