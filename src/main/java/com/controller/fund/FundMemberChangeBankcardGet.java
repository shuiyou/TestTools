...
import com.fund.bean.FundMemberChangeBankcardGetBean;
 ...
 */
@Controller
public class FundMemberChangeBankcardGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberChangeBankcardGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmcbgBean") FundMemberChangeBankcardGetBean fmcbgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmcbgBean.getChange_bankcard_no().isEmpty()) {
            inputString.append(",change_bankcard_no=" + fmcbgBean.getChange_bankcard_no());
        }
        ...
        fmcbgBean.setEncodeString(sortString);
        String requestString = fmcbgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
