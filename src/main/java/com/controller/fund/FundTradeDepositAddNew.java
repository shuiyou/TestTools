...
import com.fund.bean.FundTradeDepositAddNewBean;
...
@Controller
public class FundTradeDepositAddNew extends HttpServlet {
    ...
    @RequestMapping(value = "FundTradeDepositAddNew")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("ftdanBean") FundTradeDepositAddNewBean ftdanBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!ftdanBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + ftdanBean.getMember_id());
        }
        if (!ftdanBean.getAmount().isEmpty()) {
            inputString.append(",amount=" + ftdanBean.getAmount());
        }

        if (!ftdanBean.getReturn_url().isEmpty()) {
            inputString.append(",return_url=" + ftdanBean.getReturn_url());
        }
        if (!ftdanBean.getUser_fee().isEmpty()) {
            inputString.append(",user_fee=" + ftdanBean.getUser_fee());
        }

        if (!ftdanBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + ftdanBean.getOut_request_no());
        }

        if (!ftdanBean.getExtension().isEmpty()) {
            inputString.append(",extension=" + ftdanBean.getExtension());
        }
        ...
        ftdanBean.setEncodeString(sortString);
        if (!ftdanBean.getReturn_url().isEmpty()) {
            ftdanBean.setEncodeString(tool.removeFromString(ftdanBean.getEncodeString(), "&", "&", "startswith", "return_url") + "&return_url=" + tool.textEncode(ftdanBean.getReturn_url(), "UTF-8"));
        }

        String requestString = ftdanBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
