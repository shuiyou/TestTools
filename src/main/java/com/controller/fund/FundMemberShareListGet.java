...
import com.fund.bean.FundMemberShareListGetBean;
 ...
 */
@Controller
public class FundMemberShareListGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundMemberShareListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmslgBean") FundMemberShareListGetBean fmslgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmslgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmslgBean.getMember_id());
        }
        if (!fmslgBean.getFund_code_list().isEmpty()) {
            inputString.append(",fund_code_list=" + fmslgBean.getFund_code_list());
        }
        if (!fmslgBean.getTrade_account_no().isEmpty()) {
            inputString.append(",trade_account_no=" + fmslgBean.getTrade_account_no());
        }
        if (!fmslgBean.getFilter_fund_code_list().isEmpty()){
            inputString.append(",filter_fund_code_list=" + fmslgBean.getFilter_fund_code_list());
        }
        ...
        fmslgBean.setEncodeString(sortString);

        String requestString = fmslgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
