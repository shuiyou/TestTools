 ...
 */
@Controller
public class FundTradeRedeemOrderAdd extends HttpServlet{
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundTradeRedeemOrderAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("ftroaBean") FundTradeRedeemOrderAddBean ftroaBean,
        ...
        if (!ftroaBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + ftroaBean.getMember_id());
        }
        if (!ftroaBean.getShare_id().isEmpty()) {
            inputString.append(",share_id=" + ftroaBean.getShare_id());
        }
        if (!ftroaBean.getTrade_account_no().isEmpty()) {
            inputString.append(",trade_account_no=" + ftroaBean.getTrade_account_no());
        }
        if (!ftroaBean.getFund_code().isEmpty()) {
            inputString.append(",fund_code=" + ftroaBean.getFund_code());
        }
        if (!ftroaBean.getPayment_type().isEmpty()) {
            inputString.append(",payment_type=" + ftroaBean.getPayment_type());
        }
        if (!ftroaBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + ftroaBean.getOut_request_no());
        }
        if (!ftroaBean.getTrade_share().isEmpty()) {
            inputString.append(",trade_share=" + ftroaBean.getTrade_share());
        }
        if (!ftroaBean.getLarge_flag().isEmpty()) {
            inputString.append(",large_flag=" + ftroaBean.getLarge_flag());
        }
        ...
        ftroaBean.setEncodeString(sortString);
        String requestString = ftroaBean.getEncodeString() + "&sign="
