...
import com.fund.bean.FundOrderGetBean;
 ...
 */
@Controller
public class FundOrderGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundOrderGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fogBean") FundOrderGetBean fogBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fogBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no=" + fogBean.getOut_request_no());
        }
        if (!fogBean.getTrade_order_no().isEmpty()) {
            inputString.append(",trade_order_no=" + fogBean.getTrade_order_no());
        }
        ...
        fogBean.setEncodeString(sortString);

        String requestString = fogBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
