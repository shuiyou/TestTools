...
import com.fund.bean.FundMemberChangeBankcardBean;
 ...
 */
@Controller
public class FundMemberChangeBankcard extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberChangeBankcard")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmcbBean") FundMemberChangeBankcardBean fmcbBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmcbBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmcbBean.getMember_id());
        }
        if (!fmcbBean.getOld_payment_method_id().isEmpty()) {
            inputString.append(",old_payment_method_id=" + fmcbBean.getOld_payment_method_id());
        }
        if (!fmcbBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + fmcbBean.getPayment_method_id());
        }
        ...
        fmcbBean.setEncodeString(sortString);
        String requestString = fmcbBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
