...
import com.fund.bean.FundInvestPlanStatusUpdateBean;
 ...
 */
@Controller
public class FundInvestPlanStatusUpdate extends HttpServlet {
    ...
    @RequestMapping(value = "FundInvestPlanStatusUpdate")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fipsuBean") FundInvestPlanStatusUpdateBean fipsuBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fipsuBean.getProtocol_no().isEmpty()) {
            inputString.append(",protocol_no=" + fipsuBean.getProtocol_no());
        }
        if (!fipsuBean.getStatus().isEmpty()) {
            inputString.append(",status=" + fipsuBean.getStatus());
        }
        if (!fipsuBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fipsuBean.getMember_id());
        }
        ...
        fipsuBean.setEncodeString(sortString);
        String requestString = fipsuBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
