...
import com.fund.bean.FundTaAcctGetBean;
 ...
 */
@Controller
public class FundTaAcctGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundTaAcctGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("ftagBean") FundTaAcctGetBean ftagBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!ftagBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + ftagBean.getMember_id());
        }
        if (!ftagBean.getTa_code().isEmpty()) {
            inputString.append(",ta_code=" + ftagBean.getTa_code());
        }
        ...
        ftagBean.setEncodeString(sortString);

        String requestString = ftagBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
