...
import com.fund.bean.FundMemberPaymentMethodListGetBean;
 ...
 */
@Controller
public class FundMemberPaymentMethodListGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberPaymentMethodListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmpmlgBean") FundMemberPaymentMethodListGetBean fmpmlgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmpmlgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmpmlgBean.getMember_id());
        }
        if (!fmpmlgBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + fmpmlgBean.getPayment_method_id());
        }
        if (!fmpmlgBean.getTrade_account_no().isEmpty()) {
            inputString.append(",trade_account_no=" + fmpmlgBean.getTrade_account_no());
        }
        if (!fmpmlgBean.getMain_card_flag().isEmpty()) {
            inputString.append(",main_card_flag=" + fmpmlgBean.getMain_card_flag());
        }
        ...
        fmpmlgBean.setEncodeString(sortString);

        String requestString = fmpmlgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
