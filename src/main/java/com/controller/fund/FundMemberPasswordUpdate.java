...
import com.fund.bean.FundMemberPasswordUpdateBean;
 ...
 */
@Controller
public class FundMemberPasswordUpdate extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberPasswordUpdate")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fmpuBean") FundMemberPasswordUpdateBean fmpuBean,
        ...
        if (!fmpuBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmpuBean.getMember_id());
        }
        String passWord = null;
        if (!fmpuBean.getPassword().isEmpty()) {
            try {
                passWord = rsa.encrypt(fmpuBean.getPassword(),
                        fundBaseBean.getEncryptkey());
                inputString.append(",password=" + passWord);
            } catch (Exception e) {
        ...
        fmpuBean.setEncodeString(sortString);
        if (!fmpuBean.getPassword().isEmpty()) {
            fmpuBean.setEncodeString(tool.removeFromString(
                    fmpuBean.getEncodeString(), "&", "&", "startswith",
                    "password")
                    + "&password="
                    + tool.textEncode(passWord, "UTF-8"));
        }

        String requestString = fmpuBean.getEncodeString() + "&sign="
