 ...
 */
@Controller
public class FundMemberBindPhoneAdd extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberBindPhoneAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fmbpaBean") FundMemberBindPhoneAddBean fmbpaBean,
        ...
        if (!fmbpaBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmbpaBean.getMember_id());
        }
        String accountName = null;
        if (!fmbpaBean.getAccount_name().isEmpty()) {
            try {
                accountName = rsa.encrypt(fmbpaBean.getAccount_name(),
        ...
        String phoneNo = null;
        if (!fmbpaBean.getPhone_no().isEmpty()) {
            try {
                phoneNo = rsa.encrypt(fmbpaBean.getPhone_no(),
                        fundBaseBean.getEncryptkey());
        ...
        fmbpaBean.setEncodeString(sortString);
        if (!fmbpaBean.getAccount_name().isEmpty()) {
            fmbpaBean.setEncodeString(tool.removeFromString(
                    fmbpaBean.getEncodeString(), "&", "&", "startswith",
                    "account_name")
                    + "&account_name="
        ...
        String requestString = fmbpaBean.getEncodeString() + "&sign="
