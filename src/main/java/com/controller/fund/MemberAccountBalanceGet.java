...
import com.fund.bean.MemberAccountBalanceGetBean;
 ...
 */
@Controller
public class MemberAccountBalanceGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "MemberAccountBalanceGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("mabgBean") MemberAccountBalanceGetBean mabgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!mabgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + mabgBean.getMember_id());
        }
        if (!mabgBean.getAccount_type().isEmpty()) {
            inputString.append(",account_type=" + mabgBean.getAccount_type());
        }
        ...
        mabgBean.setEncodeString(sortString);

        String requestString = mabgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
