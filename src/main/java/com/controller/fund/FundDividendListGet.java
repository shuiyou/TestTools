...
import com.fund.bean.FundDividendListGetBean;
 ...
 */
@Controller
public class FundDividendListGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundDividendListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fdlgBean") FundDividendListGetBean fdlgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fdlgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fdlgBean.getMember_id());
        }
        if (!fdlgBean.getFund_code_list().isEmpty()) {
            inputString.append(",fund_code_list=" + fdlgBean.getFund_code_list());
        }
        if (!fdlgBean.getFilter_fund_code_list().isEmpty()){
            inputString.append(",filter_fund_code_list=" + fdlgBean.getFilter_fund_code_list());
        }
        if (!fdlgBean.getTrade_account_no().isEmpty()) {
            inputString.append(",trade_account_no=" + fdlgBean.getTrade_account_no());
        }
        ...
        fdlgBean.setEncodeString(sortString);

        String requestString = fdlgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
