...
import com.fund.bean.FundMemberCardBindValidatePushBean;
 ...
 */
@Controller
public class FundMemberCardBindValidatePush extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberCardBindValidatePush")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmcbvpBean") FundMemberCardBindValidatePushBean fmcbvpBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmcbvpBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmcbvpBean.getMember_id());
        }
        if (!fmcbvpBean.getPayment_id().isEmpty()) {
            inputString.append(",payment_id=" + fmcbvpBean.getPayment_id());
        }
        if (!fmcbvpBean.getValid_code().isEmpty()) {
            inputString.append(",valid_code=" + fmcbvpBean.getValid_code());
        }
        ...
        fmcbvpBean.setEncodeString(sortString);
        String requestString = fmcbvpBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
