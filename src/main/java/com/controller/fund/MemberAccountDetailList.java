...
import com.fund.bean.MemberAccountDetailListBean;
 ...
 */
@Controller
public class MemberAccountDetailList extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "MemberAccountDetailList")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("madlBean") MemberAccountDetailListBean madlBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!madlBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + madlBean.getMember_id());
        }
        if (!madlBean.getAccount_type().isEmpty()) {
            inputString.append(",account_type=" + madlBean.getAccount_type());
        }
        if (!madlBean.getBusiness_types().isEmpty()) {
            inputString.append(",business_types=" + madlBean.getBusiness_types());
        }
        if (!madlBean.getStart_time().isEmpty()) {
            inputString.append(",start_time=" + madlBean.getStart_time());
        }
        if (!madlBean.getEnd_time().isEmpty()) {
            inputString.append(",end_time=" + madlBean.getEnd_time());
        }
        if (!madlBean.getPage_no().isEmpty()) {
            inputString.append(",page_no=" + madlBean.getPage_no());
        }
        if (!madlBean.getPage_size().isEmpty()) {
            inputString.append(",page_size=" + madlBean.getPage_size());
        }
        ...
        madlBean.setEncodeString(sortString);

        String requestString = madlBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
