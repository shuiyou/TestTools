...
import com.fund.bean.FundMemberPasswordCheckBean;
 ...
 */
@Controller
public class FundMemberPasswordCheck extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberPasswordCheck")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fmpcBean") FundMemberPasswordCheckBean fmpcBean,
        ...
        if (!fmpcBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmpcBean.getMember_id());
        }
        String passWord = null;
        if (!fmpcBean.getPassword().isEmpty()) {
            try {
                passWord = rsa.encrypt(fmpcBean.getPassword(),
        ...
        fmpcBean.setEncodeString(sortString);
        if (!fmpcBean.getPassword().isEmpty()) {
            fmpcBean.setEncodeString(tool.removeFromString(
                    fmpcBean.getEncodeString(), "&", "&", "startswith",
                    "password")
                    + "&password="
                    + tool.textEncode(passWord, "UTF-8"));
        }

        String requestString = fmpcBean.getEncodeString() + "&sign="
