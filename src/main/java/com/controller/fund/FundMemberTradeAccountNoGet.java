...
import com.fund.bean.FundMemberTradeAccountNoGetBean;
 ...
 */
@Controller
public class FundMemberTradeAccountNoGet extends HttpServlet {
    /**
     *
     */
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundMemberTradeAccountNoGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmtangBean") FundMemberTradeAccountNoGetBean fmtangBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmtangBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmtangBean.getMember_id());
        }
        if (!fmtangBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + fmtangBean.getPayment_method_id());
        }
        if (!fmtangBean.getFund_code().isEmpty()) {
            inputString.append(",fund_code=" + fmtangBean.getFund_code());
        }
        if (!fmtangBean.getPortfolio_code().isEmpty()) {
            inputString.append(",portfolio_code=" + fmtangBean.getPortfolio_code());
        }
        ...
        fmtangBean.setEncodeString(sortString);


        String requestString = fmtangBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
