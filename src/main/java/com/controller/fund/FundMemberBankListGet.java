...
import com.fund.bean.FundMemberBankListGetBean;
 ...
 */
@Controller
public class FundMemberBankListGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberBankListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmblgBean") FundMemberBankListGetBean fmblgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmblgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmblgBean.getMember_id());
        }
        ...
        fmblgBean.setEncodeString(sortString);

        String requestString = fmblgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
