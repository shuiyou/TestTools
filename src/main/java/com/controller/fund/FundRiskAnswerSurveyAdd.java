 ...
 */
@Controller
public class FundRiskAnswerSurveyAdd extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundRiskAnswerSurveyAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("frasaBean") FundRiskAnswerSurveyAddBean frasaBean,
        ...
        if (!frasaBean.getMember_id().isEmpty()) {
            inputString.append(";member_id=" + frasaBean.getMember_id());
        }
        if (!frasaBean.getSelections().isEmpty()) {
            inputString.append(";selections=" + frasaBean.getSelections());
        }
