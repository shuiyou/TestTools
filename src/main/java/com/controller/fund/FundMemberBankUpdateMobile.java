...
import com.fund.bean.FundMemberBankUpdateMobileBean;
import com.fund.bean.FundMemberBindPhoneAddBean;
 ...
 */
@Controller
public class FundMemberBankUpdateMobile extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberBankUpdateMobile")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("fmbumBean") FundMemberBankUpdateMobileBean fmbumBean,
        ...
        if (!fmbumBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmbumBean.getMember_id());
        }
        if (!fmbumBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + fmbumBean.getPayment_method_id());
        }

        String phoneNo = null;
        if (!fmbumBean.getPhone_no().isEmpty()) {
            try {
                phoneNo = rsa.encrypt(fmbumBean.getPhone_no(),
        ...
        fmbumBean.setEncodeString(sortString);
        if (!fmbumBean.getPhone_no().isEmpty()) {
            fmbumBean.setEncodeString(tool.removeFromString(
                    fmbumBean.getEncodeString(), "&", "&", "startswith",
                    "phone_no")
                    + "&phone_no="
                    + tool.textEncode(phoneNo, "UTF-8"));
        }

        String requestString = fmbumBean.getEncodeString() + "&sign="
