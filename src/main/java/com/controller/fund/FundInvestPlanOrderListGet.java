...
import com.fund.bean.FundInvestPlanOrderListGetBean;
 ...
 */
@Controller
public class FundInvestPlanOrderListGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundInvestPlanOrderListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fipolgBean") FundInvestPlanOrderListGetBean fipolgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fipolgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fipolgBean.getMember_id());
        }
        if (!fipolgBean.getProtocol_no().isEmpty()) {
            inputString.append(",protocol_no=" + fipolgBean.getProtocol_no());
        }
        if (!fipolgBean.getPay_status_list().isEmpty()) {
            inputString.append(",pay_status_list=" + fipolgBean.getPay_status_list());
        }
        if (!fipolgBean.getConfirm_status_list().isEmpty()) {
            inputString.append(",confirm_status_list=" + fipolgBean.getConfirm_status_list());
        }
        ...
        fipolgBean.setEncodeString(sortString);


        String requestString = fipolgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
