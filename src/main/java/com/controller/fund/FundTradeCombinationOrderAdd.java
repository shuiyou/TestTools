...
import com.fund.bean.FundTradeCombinationOrderAddBean;
 ...
 */
@Controller
public class FundTradeCombinationOrderAdd extends HttpServlet{
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundTradeCombinationOrderAdd")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("ftboaBean") FundTradeCombinationOrderAddBean ftcoaBean,
        ...
        if (!ftcoaBean.getPayment_method_id().isEmpty()) {
            inputString.append(",payment_method_id=" + ftcoaBean.getPayment_method_id());
        }
        if (!ftcoaBean.getPayment_type().isEmpty()) {
            inputString.append(",payment_type=" + ftcoaBean.getPayment_type());
        }
        if (!ftcoaBean.getPortfolio_apply_no().isEmpty()) {
            inputString.append(",portfolio_apply_no=" + ftcoaBean.getPortfolio_apply_no());
        }
        if (!ftcoaBean.getPortfolio_code().isEmpty()) {
            inputString.append(",portfolio_code=" + ftcoaBean.getPortfolio_code());
        }
        if (!ftcoaBean.getCombination_details().isEmpty()) {
            inputString.append(",combination_details=" + ftcoaBean.getCombination_details().replace(",","replaceFlag"));
        }
        ...
        if (!ftcoaBean.getCombination_details().isEmpty()) {
            ftcoaBean.setEncodeString(tool.removeFromString(ftcoaBean.getEncodeString(), "&", "&", "startswith", "combination_details") + "&combination_details=" + tool.textEncode(ftcoaBean.getCombination_details(), "UTF-8"));
        }



        String requestString = ftcoaBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
