...
import com.fund.bean.FundTradeOrderCancelBean;
 ...
 */
@Controller
public class FundTradeOrderCancel extends HttpServlet{
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundTradeOrderCancel")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean,
                       @ModelAttribute("ftocBean") FundTradeOrderCancelBean ftocBean,
        ...
        if (!ftocBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + ftocBean.getMember_id());
        }

        if (!ftocBean.getOrg_trade_order_no().isEmpty()) {
            inputString.append(",org_trade_order_no=" + ftocBean.getOrg_trade_order_no());
        }
        if (!ftocBean.getWhether_combination().isEmpty()) {
            inputString.append(",whether_combination=" + ftocBean.getWhether_combination());
        }
        ...
        ftocBean.setEncodeString(sortString);
        String requestString = ftocBean.getEncodeString() + "&sign="
