...
import com.fund.bean.FundProductListGetBean;
 ...
 */
@Controller
public class FundProductListGet extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "FundProductListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fplgBean") FundProductListGetBean fplgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fplgBean.getManager_code().isEmpty()) {
            inputString.append(",manager_code=" + fplgBean.getManager_code());
        }
        if (!fplgBean.getFund_status().isEmpty()) {
            inputString.append(",fund_status=" + fplgBean.getFund_status());
        }
        if (!fplgBean.getFund_codes().isEmpty()) {
            inputString.append(",fund_codes=" + fplgBean.getFund_codes());
        }
        if (!fplgBean.getFund_type().isEmpty()) {
            inputString.append(",fund_type=" + fplgBean.getFund_type());
        }
        if (!fplgBean.getRisk_level().isEmpty()) {
            inputString.append(",risk_level=" + fplgBean.getRisk_level());
        }
        ...
        fplgBean.setEncodeString(sortString);

        String requestString = fplgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
