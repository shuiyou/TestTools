 ...
 */
@Controller
public class FundInvestPlanListGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundInvestPlanListGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fiplgBean") FundInvestPlanListGetBean fiplgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fiplgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fiplgBean.getMember_id());
        }
        if (!fiplgBean.getProtocol_no().isEmpty()) {
            inputString.append(",protocol_no=" + fiplgBean.getProtocol_no());
        }
        if (!fiplgBean.getStatus().isEmpty()) {
            inputString.append(",status=" + fiplgBean.getStatus());
        }
        ...
        fiplgBean.setEncodeString(sortString);
        String requestString = fiplgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
