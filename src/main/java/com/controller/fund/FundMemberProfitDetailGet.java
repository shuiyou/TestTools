...
import com.fund.bean.FundMemberProfitDetailGetBean;
 ...
 */
@Controller
public class FundMemberProfitDetailGet extends HttpServlet {
    ...
    @RequestMapping(value = "FundMemberProfitDetailGet")
    public void doPost(@ModelAttribute("FundBaseBean") FundBaseBean fundBaseBean, @ModelAttribute("fmpdgBean") FundMemberProfitDetailGetBean fmpdgBean, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!fmpdgBean.getMember_id().isEmpty()) {
            inputString.append(",member_id=" + fmpdgBean.getMember_id());
        }
        ...
        fmpdgBean.setEncodeString(sortString);
        String requestString = fmpdgBean.getEncodeString() + "&sign=" + tool.textEncode(sign, "UTF-8");
