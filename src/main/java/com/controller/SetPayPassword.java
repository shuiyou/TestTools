...
import com.mgs.bean.SetPayPasswordBean;
 ...
 */
@Controller
public class SetPayPassword extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("camBean") SetPayPasswordBean sppBean,
		...
		if (!sppBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + sppBean.getIdentity_id());
		}
		if (!sppBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + sppBean.getIdentity_type());
		}
		...
		if (!sppBean.getClient_ip().isEmpty() && sppBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + sppBean.getClient_ip().replace(",", "#"));
		}else if (!sppBean.getClient_ip().isEmpty() && !sppBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + sppBean.getClient_ip());
		}
		if (!sppBean.getWithhold_param().isEmpty() && sppBean.getWithhold_param().contains(",")) {
			inputString.append(",withhold_param=" + sppBean.getWithhold_param().replace(",", "#"));
		}else if (!sppBean.getWithhold_param().isEmpty() && !sppBean.getWithhold_param().contains(",")){
			inputString.append(",withhold_param=" + sppBean.getWithhold_param());
		}
		...
		if (!sppBean.getWithhold_param().isEmpty()) {
			sppBean.setEncodeString(tool.removeFromString(
					sppBean.getEncodeString(), "&", "&", "startswith",
					"withhold_param")
					+ "&withhold_param="
					+ tool.textEncode(sppBean.getWithhold_param(), "UTF-8"));
		}
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			sppBean.setEncodeString(tool.removeFromString(
					sppBean.getEncodeString(), "&", "&", "startswith",
