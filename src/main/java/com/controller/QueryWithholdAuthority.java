...
import com.mgs.bean.QueryWithholdAuthorityBean;
 ...
 */
@Controller
public class QueryWithholdAuthority extends HttpServlet {
	...
	@RequestMapping(value = "QueryWithholdAuthority")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("hwaBean") QueryWithholdAuthorityBean qwaBean,
		...
		if (!qwaBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + qwaBean.getIdentity_id());
		}
		if (!qwaBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + qwaBean.getIdentity_type());
		}
		if (!qwaBean.getAuth_type().isEmpty() ) {
			inputString.append(",auth_type=" + qwaBean.getAuth_type());
		}
		if (!qwaBean.getAuth_sub_type().isEmpty() ) {
			inputString.append(",auth_sub_type=" + qwaBean.getAuth_sub_type());
		}
		if (!qwaBean.getIs_detail_disp().isEmpty() ) {
			inputString.append(",is_detail_disp=" + qwaBean.getIs_detail_disp());
		}
		if (!qwaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qwaBean.getExtend_param());
		}
		...
		qwaBean.setEncodeString(sortString);
		
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			qwaBean.setEncodeString(tool.removeFromString(
					qwaBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qwaBean.getEncodeString() + "&sign="
