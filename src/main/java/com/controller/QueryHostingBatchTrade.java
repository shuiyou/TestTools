...
import com.mas.bean.QueryHostingBatchTradeBean;
 ...
 */
@Controller
public class QueryHostingBatchTrade extends HttpServlet {
	...
	@RequestMapping(value = "QueryHostingBatchTrade")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("qhbtBean") QueryHostingBatchTradeBean qhbtBean,
		...
		if (!qhbtBean.getOut_batch_no().isEmpty()) {
			inputString.append(",out_batch_no=" + qhbtBean.getOut_batch_no());
		}
		if (!qhbtBean.getPage_no().isEmpty()) {
			inputString.append(",page_no=" + qhbtBean.getPage_no());
		}
		if (!qhbtBean.getPage_size().isEmpty()) {
			inputString.append(",page_size=" + qhbtBean.getPage_size());
		}
		...
		qhbtBean.setEncodeString(sortString);
		if (!b2cBaseBean.getNotify_url().isEmpty()) {
			qhbtBean.setEncodeString(tool.removeFromString(
					qhbtBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qhbtBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
