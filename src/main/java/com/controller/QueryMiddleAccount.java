...
import com.mgs.bean.QueryMiddleAccountBean;
 ...
 */
@Controller
public class QueryMiddleAccount extends HttpServlet {
	...
	@RequestMapping(value = "QueryMiddleAccount")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("qmaBean") QueryMiddleAccountBean qmaBean,
		...
		if (!qmaBean.getOut_trade_code().isEmpty()) {
			inputString.append(",out_trade_code=" + qmaBean.getOut_trade_code());
		}
		if (!qmaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qmaBean.getExtend_param());
		}
		...
		qmaBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			qmaBean.setEncodeString(tool.removeFromString(
					qmaBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qmaBean.getEncodeString() + "&sign="
