...
import com.mgs.bean.ValidatePayPasswordBean;
 ...
 */
@Controller
public class ValidatePayPassword extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("vppBean") ValidatePayPasswordBean vppBean,
		...
		if (!vppBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + vppBean.getIdentity_id());
		}
		if (!vppBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + vppBean.getIdentity_type());
		}
		if (!vppBean.getRequest_no().isEmpty()) {
			inputString.append(",request_no=" + vppBean.getRequest_no());
		}
		if (!vppBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + vppBean.getExtend_param());
		}
		if (!vppBean.getCashdesk_addr_category().isEmpty()) {
			inputString.append(",cashdesk_addr_category=" + vppBean.getCashdesk_addr_category());
		}
		...
		vppBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			vppBean.setEncodeString(tool.removeFromString(
					vppBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = vppBean.getEncodeString() + "&sign="
