...
import com.mas.bean.FinishPreAuthTradeBean;
 ...
 */
@Controller
public class FinishPreAuthTrade extends HttpServlet {
	...
	@RequestMapping(value = "FinishPreAuthTrade")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("phtBean") FinishPreAuthTradeBean phtBean,
		...
		if (!phtBean.getOut_request_no().isEmpty()) {
			inputString.append(",out_request_no=" + phtBean.getOut_request_no());
		}
		if (!phtBean.getTrade_list().isEmpty()) {
			inputString.append(",trade_list="
					+ phtBean.getTrade_list());
		}
		if (!phtBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + phtBean.getExtend_param());
		}
		if (!phtBean.getUser_ip().isEmpty()) {
			inputString.append(",user_ip=" + phtBean.getUser_ip());
		}
		...
		String requestString = phtBean.getEncodeString()+ "&sign=" 
