...
import com.mgs.bean.BindingBankCardAdvanceBean;
import com.mgs.bean.ChangeBankMobileAdvanceBean;
 ...
 */
@Controller
public class ChangeBankMobileAdvance extends HttpServlet {
	...
	@RequestMapping(value = "ChangeBankMobileAdvance")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("bbcaBean") ChangeBankMobileAdvanceBean bbcaBean,
		...
		if (!bbcaBean.getTicket().isEmpty()) {
			inputString.append(",ticket=" + bbcaBean.getTicket());
		}
		if (!bbcaBean.getValid_code().isEmpty()) {
			inputString.append(",valid_code=" + bbcaBean.getValid_code());
		}
		if (!bbcaBean.getClient_ip().isEmpty() && bbcaBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + bbcaBean.getClient_ip().replace(",", "#"));
		}else if (!bbcaBean.getClient_ip().isEmpty() && !bbcaBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + bbcaBean.getClient_ip());
		}
		if (!bbcaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + bbcaBean.getExtend_param());
		}
		...
		bbcaBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			bbcaBean.setEncodeString(tool.removeFromString(
					bbcaBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = bbcaBean.getEncodeString() + "&sign="
