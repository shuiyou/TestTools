...
import com.egs.bean.EgsChangeBindingCardBean;


@Controller
public class EgsChangeBindingCard extends HttpServlet{
	
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "EgsChangeBindingCard")
	public void doPost(@ModelAttribute("egsBaseBean") EgsBaseBean egsBaseBean,
			@ModelAttribute("camBean") EgsChangeBindingCardBean camBean,
		...
		if (!camBean.getOrig_card_id().isEmpty()) {
			inputString.append(",orig_card_id=" + camBean.getOrig_card_id());
		}
		if (!camBean.getCard_id().isEmpty()) {
			inputString.append(",card_id=" + camBean.getCard_id());
		}
