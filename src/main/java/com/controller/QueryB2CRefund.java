...
import com.b2c.bean.QueryB2CRefundBean;
 ...
 */
@Controller
public class QueryB2CRefund extends HttpServlet {
	...
	@RequestMapping(value = "QueryB2CRefund")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("qrBean") QueryB2CRefundBean qrBean,
		...
		if (!qrBean.getExtend_param().isEmpty()) {
			qrBean.setEncodeString(tool.removeFromString(
					...
					"extend_param")
					+ "&extend_param="
					+ tool.textEncode(qrBean.getExtend_param(), "UTF-8"));
		}
