...
import com.pas.bean.QueryCarInfoBean;

@Controller
public class QueryCarInfo extends HttpServlet{
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "QueryCarInfo")
    public void doPost(@ModelAttribute("pasBaseBean") PasBaseBean pasBaseBean,
	    @ModelAttribute("vbcBean") QueryCarInfoBean vbcBean,
	...
	if (!vbcBean.getLicenseNo().isEmpty()) {
	    inputString
		    .append(",licenseNo=" + vbcBean.getLicenseNo());
	}
	
	if (!vbcBean.getCarType().isEmpty()) {
	    inputString.append(",carType=" + vbcBean.getCarType());
	}
