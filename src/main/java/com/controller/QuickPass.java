...
import com.mgs.bean.CreateActivateMemberBean;
import com.mgs.bean.MgsBaseBean;
import com.mgs.bean.QuickPassBean;
 ...
 */
@Controller
public class QuickPass extends HttpServlet {
	...
	@RequestMapping(value = "QuickPass")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("camBean") QuickPassBean camBean,
		...
		if (!camBean.getCancel_url().isEmpty()) {
			inputString.append(",cancel_url=" + camBean.getCancel_url());
		}
		if (!camBean.getClient_ip().isEmpty() && camBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + camBean.getClient_ip().replace(",", "#"));
		}else if (!camBean.getClient_ip().isEmpty() && !camBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + camBean.getClient_ip());
		}
		if (!camBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + camBean.getExtend_param());
		}

		if (!camBean.getDevice_id().isEmpty()) {
			inputString.append(",device_id=" + camBean.getDevice_id());
		}
