...
@Controller
public class ReceiptSplit extends HttpServlet {
 ...
 */
	...
	@RequestMapping(value = "ReceiptSplit")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("cboBean") ReceiptSplitBean cboBean,
		...
		if (!cboBean.getOrig_out_trade_no().isEmpty()) {
			inputString.append(",orig_out_trade_no=" + cboBean.getOrig_out_trade_no());
		}
		if (!cboBean.getSplit_list().isEmpty()) {
			inputString.append(",split_list="
					+ cboBean.getSplit_list());
		}
		if (!cboBean.getSplit_type().isEmpty()) {
			inputString.append(",split_type="
					+ cboBean.getSplit_type());
		}
