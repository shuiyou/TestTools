...
import com.mgs.bean.WebMemberInfosBean;
 ...
 */
@Controller
public class WebMemberInfos extends HttpServlet {
    ...
    @RequestMapping(value = "WebMemberInfosAuth")
    public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
                       @ModelAttribute("wmiBean") WebMemberInfosBean wmiBean,
        ...
        if (!wmiBean.getIdentity_id().isEmpty()) {
            inputString.append(",identity_id=" + wmiBean.getIdentity_id());
        }
        if (!wmiBean.getIdentity_type().isEmpty()) {
            inputString.append(",identity_type=" + wmiBean.getIdentity_type());
        }
        ...
        if (!mgsBaseBean.getNotify_url().isEmpty()) {
            wmiBean.setEncodeString(tool.removeFromString(
                    wmiBean.getEncodeString(), "&", "&", "startswith",
                    "notify_url")
                    + "&notify_url="
        ...
        String requestString = wmiBean.getEncodeString() + "&sign="
