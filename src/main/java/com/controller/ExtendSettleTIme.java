...
import com.mas.bean.ExtendSettleTimeBean;
 ...
 */
@Controller
public class ExtendSettleTIme extends HttpServlet {
    private static final long serialVersionUID = 1L;

    @RequestMapping(value = "ExtendSettleTime")
    public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
                       @ModelAttribute("ExtendSettleTimeBean") ExtendSettleTimeBean ExtendSettleTimeBean,
        ...
        if (!ExtendSettleTimeBean.getOut_trade_no().isEmpty()) {
            inputString.append(",out_trade_no=" + ExtendSettleTimeBean.getOut_trade_no());
        }
        if (!ExtendSettleTimeBean.getOut_request_no().isEmpty()) {
            inputString.append(",out_request_no="
                    + ExtendSettleTimeBean.getOut_request_no());
        }
        if (!ExtendSettleTimeBean.getExtend_time().isEmpty()) {
            inputString.append(",extend_time="
                    + ExtendSettleTimeBean.getExtend_time());
        }
        if (!ExtendSettleTimeBean.getExtend_param().isEmpty()) {
            inputString.append(",extend_param="
                    + ExtendSettleTimeBean.getExtend_param());
        }
        if (!ExtendSettleTimeBean.getSummary().isEmpty()) {
            inputString.append(",summary="
                    + ExtendSettleTimeBean.getSummary());
        }
        ...
        ExtendSettleTimeBean.setEncodeString(sortString);
        if (!masBaseBean.getNotify_url().isEmpty()) {
            ExtendSettleTimeBean.setEncodeString(tool.removeFromString(
                    ExtendSettleTimeBean.getEncodeString(), "&", "&", "startswith",
                    "notify_url")
                    + "&notify_url="
        ...
        String requestString = ExtendSettleTimeBean.getEncodeString().replaceAll(
                "replaceFlag", ",")
