...
import com.b2c.bean.QueryReceiptSplitBean;
import com.b2c.bean.ReceiptSplitBean;
...
@Controller
public class QueryReceiptSplit extends HttpServlet {
 ...
 */
	...
	@RequestMapping(value = "QueryReceiptSplit")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("cboBean") QueryReceiptSplitBean cboBean,
		...
		if (!cboBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + cboBean.getIdentity_id());
		}
		if (!cboBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type="
					+ cboBean.getIdentity_type());
		}
		if (!cboBean.getOut_split_no().isEmpty()) {
			inputString.append(",out_split_no="
					+ cboBean.getOut_split_no());
		}
		if (!cboBean.getStart_time().isEmpty()) {
			inputString.append(",start_time="
					+ cboBean.getStart_time());
		}
		if (!cboBean.getEnd_time().isEmpty()) {
			inputString.append(",end_time="
					+ cboBean.getEnd_time());
		}
		if (!cboBean.getPage_no().isEmpty()) {
			inputString.append(",page_no="
					+ cboBean.getPage_no());
		}
		if (!cboBean.getPage_size().isEmpty()) {
			inputString.append(",page_size="
					+ cboBean.getPage_size());
		}
