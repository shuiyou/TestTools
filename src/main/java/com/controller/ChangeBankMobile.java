...
import com.mgs.bean.ChangeBankMobileBean;
 ...
 */
@Controller
public class ChangeBankMobile extends HttpServlet {
	...
	@RequestMapping(value = "ChangeBankMobile")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("bvBean") ChangeBankMobileBean bvBean,
		...
		if (!bvBean.getRequest_no().isEmpty()) {
			inputString.append(",request_no=" + bvBean.getRequest_no());
		}
		if (!bvBean.getCard_id().isEmpty()) {
			inputString.append(",card_id=" + bvBean.getCard_id());
		}
		
		if (!bvBean.getPhone_no().isEmpty()) {
			try {
				encryptPhoneNo = rsa.encrypt(bvBean.getPhone_no(),
