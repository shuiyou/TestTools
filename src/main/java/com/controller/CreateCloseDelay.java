...
import com.b2c.bean.CreateCloseDelayBean;
 ...
 */
@Controller
public class CreateCloseDelay extends HttpServlet {
	...
	@RequestMapping(value = "CreateCloseDelay")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("ccdBean") CreateCloseDelayBean ccdBean,
		...
		if (!ccdBean.getOut_request_no().isEmpty()) {
			inputString.append(",out_request_no=" + ccdBean.getOut_request_no());
		}
		if (!ccdBean.getOut_trade_no().isEmpty()) {
			inputString.append(",out_trade_no=" + ccdBean.getOut_trade_no());
		}
		if (!ccdBean.getDelay_time().isEmpty()) {
			inputString.append(",delay_time=" + ccdBean.getDelay_time());
		}
		if (!ccdBean.getSummary().isEmpty()) {
			inputString.append(",summary=" + ccdBean.getSummary());
		}
		if (!ccdBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + ccdBean.getExtend_param());
		}
		...
		ccdBean.setEncodeString(sortString);
		if (!b2cBaseBean.getNotify_url().isEmpty()) {
			ccdBean.setEncodeString(tool.removeFromString(
					ccdBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		if (!ccdBean.getSummary().isEmpty()) {
			ccdBean.setEncodeString(tool.removeFromString(
					...
					"summary")
					+ "&summary="
					+ tool.textEncode(ccdBean.getSummary(), "UTF-8"));
		}
		if (!ccdBean.getExtend_param().isEmpty()) {
			ccdBean.setEncodeString(tool.removeFromString(
					ccdBean.getEncodeString(), "&", "&", "startswith",
					"extend_param")
					+ "&extend_param="
					+ tool.textEncode(ccdBean.getExtend_param(), "UTF-8"));
		}

		String requestString = ccdBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
