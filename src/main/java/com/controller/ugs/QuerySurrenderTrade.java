...
import com.ugs.bean.QuerySurrenderTradeBean;
 ...
 */
@Controller
public class QuerySurrenderTrade extends HttpServlet {
	...
	@RequestMapping(value = "QuerySurrenderTrade")
	public void doPost(@ModelAttribute("ugsBaseBean") UgsBaseBean ugsBaseBean,
			@ModelAttribute("qstBean") QuerySurrenderTradeBean qstBean,
		...
		if (!qstBean.getIdentity_id().isEmpty()) {
			biz.append("\"identity_id\":\"" + qstBean.getIdentity_id()+ "\"");
		}
		if (!qstBean.getIdentity_type().isEmpty()) {
			biz.append(",\"identity_type\":\"" + qstBean.getIdentity_type() + "\"");
		}
		if (!qstBean.getOut_trade_no().isEmpty()) {
			biz.append(",\"out_trade_no\":\"" + qstBean.getOut_trade_no()+ "\"");
		}
		if (!qstBean.getStart_time().isEmpty()) {
			biz.append(",\"start_time\":\"" + qstBean.getStart_time()+ "\"");
		}
		if (!qstBean.getEnd_time().isEmpty()) {
			biz.append(",\"end_time\":\"" + qstBean.getEnd_time()+ "\"");
		}
		if (!qstBean.getPage_no().isEmpty()) {
			biz.append(",\"page_no\":\"" + qstBean.getPage_no()+ "\"");
		}
		if (!qstBean.getPage_size().isEmpty()) {
			biz.append(",\"page_size\":\"" + qstBean.getPage_size()+ "\"");
		}
		if (!qstBean.getExtend_param().isEmpty()) {
			biz.append(",\"extend_param\":\"" + qstBean.getExtend_param()+ "\"");
		}
		...
		qstBean.setEncodeString(sortString);
//		if (!ugsBaseBean.getNotify_url().isEmpty()) {
//			qstBean.setEncodeString(tool.removeFromString(
//					qstBean.getEncodeString(), "&", "&", "startswith",
//					"notify_url")
//					+ "&notify_url="
...
//		}
		...
		String requestString = qstBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
				+ "&sign=" + sign;
