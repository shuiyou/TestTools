...
import com.ugs.bean.QueryInsureTradeBean;
 ...
 */
@Controller
public class QueryInsureTrade extends HttpServlet {
	...
	@RequestMapping(value = "QueryInsureTrade")
	public void doPost(@ModelAttribute("ugsBaseBean") UgsBaseBean ugsBaseBean,
			@ModelAttribute("qitBean") QueryInsureTradeBean qitBean,
		...
		if (!qitBean.getIdentity_id().isEmpty()) {
			biz.append("\"identity_id\":\"" + qitBean.getIdentity_id()+ "\"");
		}
		if (!qitBean.getIdentity_type().isEmpty()) {
			biz.append(",\"identity_type\":\"" + qitBean.getIdentity_type() + "\"");
		}
		if (!qitBean.getOut_trade_no().isEmpty()) {
			biz.append(",\"out_trade_no\":\"" + qitBean.getOut_trade_no()+ "\"");
		}
		if (!qitBean.getStart_time().isEmpty()) {
			biz.append(",\"start_time\":\"" + qitBean.getStart_time()+ "\"");
		}
		if (!qitBean.getEnd_time().isEmpty()) {
			biz.append(",\"end_time\":\"" + qitBean.getEnd_time()+ "\"");
		}
		if (!qitBean.getPage_no().isEmpty()) {
			biz.append(",\"page_no\":\"" + qitBean.getPage_no()+ "\"");
		}
		if (!qitBean.getPage_size().isEmpty()) {
			biz.append(",\"page_size\":\"" + qitBean.getPage_size()+ "\"");
		}
		if (!qitBean.getExtend_param().isEmpty()) {
			biz.append(",\"extend_param\":\"" + qitBean.getExtend_param()+ "\"");
		}
		...
		qitBean.setEncodeString(sortString);
//		if (!ugsBaseBean.getNotify_url().isEmpty()) {
//			qitBean.setEncodeString(tool.removeFromString(
//					qitBean.getEncodeString(), "&", "&", "startswith",
//					"notify_url")
//					+ "&notify_url="
...
//		}
		...
		String requestString = qitBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
				+ "&sign=" + sign;
