...
import com.ugs.bean.CrossQueryInboundBatchBean;
 ...
 */
@Controller
public class CrossQueryInboundBatch extends HttpServlet {
	/**
	 *
	 */
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "CrossQueryInboundBatch")
	public void doPost(@ModelAttribute("ugsBaseBean") UgsBaseBean ugsBaseBean,
			@ModelAttribute("cqibBean") CrossQueryInboundBatchBean cqibBean,
			...
			throws IOException, ServletException
	{
		...
		if (!cqibBean.getBatch_date().isEmpty()) {
			biz.append("\"batch_date\":\"" + cqibBean.getBatch_date()+ "\",");
		}

		if (!cqibBean.getBatch_no().isEmpty()) {
			biz.append("\"batch_no\":\"" + cqibBean.getBatch_no() + "\",");
		}
		...
		cqibBean.setEncodeString(sortString);
		String requestString = cqibBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
				+ "&sign=" + sign;
