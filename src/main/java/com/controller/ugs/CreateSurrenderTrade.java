...
import com.ugs.bean.CreateSurrenderTradeBean;
 ...
 */
@Controller
public class CreateSurrenderTrade extends HttpServlet {
	...
	@RequestMapping(value = "CreateSurrenderTrade")
	public void doPost(@ModelAttribute("ugsBaseBean") UgsBaseBean ugsBaseBean,
			@ModelAttribute("cstBean") CreateSurrenderTradeBean cstBean,
		...
		if (!cstBean.getOut_pay_no().isEmpty()) {
			biz.append("\"out_pay_no\":\"" + cstBean.getOut_pay_no()+ "\"");
		}
		if (!cstBean.getSummary().isEmpty()) {
			biz.append(",\"summary\":\"" + cstBean.getSummary() + "\"");
		}
		if (!cstBean.getOrig_outer_trade_no().isEmpty()) {
			biz.append(",\"orig_outer_trade_no\":\"" + cstBean.getOrig_outer_trade_no()+ "\"");
		}
		if (!cstBean.getAmount().isEmpty()) {
			biz.append(",\"amount\":\"" + cstBean.getAmount()+ "\"");
		}
		if (!cstBean.getUser_ip().isEmpty()) {
			biz.append(",\"user_ip\":\"" + cstBean.getUser_ip()+ "\"");
		}
		if (!cstBean.getSurrender_detail().isEmpty()) {
			String tmp = cstBean.getSurrender_detail().replace(",", "surrender_detail");
			biz.append(",\"surrender_detail\":\"" + tmp + "\"");
		}
		
		if (!cstBean.getExtend_param().isEmpty()) {
			biz.append(",\"extend_param\":\"" + cstBean.getExtend_param()+ "\"");
		}
		...
		cstBean.setEncodeString(sortString);
//		if (!ugsBaseBean.getNotify_url().isEmpty()) {
//			cstBean.setEncodeString(tool.removeFromString(
//					cstBean.getEncodeString(), "&", "&", "startswith",
//					"notify_url")
//					+ "&notify_url="
...
//		}
		...
		String requestString = cstBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
				+ "&sign=" + sign;
