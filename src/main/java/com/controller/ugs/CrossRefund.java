...
import com.ugs.bean.CrossRefundBean;
 ...
 */
@Controller
public class CrossRefund extends HttpServlet
{
	/**
	 *
	 */
	...
	@RequestMapping(value = "CrossRefund")
	public void doPost(@ModelAttribute("ugsBaseBean") UgsBaseBean ugsBaseBean,
			@ModelAttribute("crBean") CrossRefundBean crBean,
			...
			throws IOException, ServletException
	{
		...
		if (!crBean.getOut_trade_no().isEmpty()) {
			biz.append("\"out_trade_no\":\"" + crBean.getOut_trade_no()+ "\",");
		}

		if (!crBean.getOrig_out_trade_no().isEmpty()) {
			biz.append("\"orig_out_trade_no\":\"" + crBean.getOrig_out_trade_no() + "\",");
		}
		if (!crBean.getSummary().isEmpty()) {
			biz.append("\"summary\":\"" + crBean.getSummary()+ "\"");
		}
