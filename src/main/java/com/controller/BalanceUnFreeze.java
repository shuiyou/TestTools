...
import com.mgs.bean.BalanceUnFreezeBean;
 ...
 */
@Controller
public class BalanceUnFreeze extends HttpServlet {
	...
	@RequestMapping(value = "BalanceUnFreeze")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("bufBean") BalanceUnFreezeBean bufBean,
		...
		if (!bufBean.getOut_unfreeze_no().isEmpty()) {
			inputString.append(",out_unfreeze_no="
					+ bufBean.getOut_unfreeze_no());
		}
		if (!bufBean.getOut_freeze_no().isEmpty()) {
			inputString.append(",out_freeze_no=" + bufBean.getOut_freeze_no());
		}
		if (!bufBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + bufBean.getIdentity_id());
		}
		if (!bufBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + bufBean.getIdentity_type());
		}
		if (!bufBean.getAmount().isEmpty()) {
			inputString.append(",amount=" + bufBean.getAmount());
		}
		if (!bufBean.getSummary().isEmpty()) {
			inputString.append(",summary=" + bufBean.getSummary());
		}
		if (!bufBean.getClient_ip().isEmpty() && bufBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + bufBean.getClient_ip().replace(",", "#"));
		}else if (!bufBean.getClient_ip().isEmpty() && !bufBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + bufBean.getClient_ip());
		}
		if (!bufBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + bufBean.getExtend_param());
		}
		...
		bufBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			bufBean.setEncodeString(tool.removeFromString(
					bufBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		if (!bufBean.getSummary().isEmpty()) {
			bufBean.setEncodeString(tool.removeFromString(
					...
					"summary")
					+ "&summary="
					+ tool.textEncode(bufBean.getSummary(), "UTF-8"));
		}

		String requestString = bufBean.getEncodeString() + "&sign="
