...
import com.mas.bean.AdvanceHostingPayBean;
 ...
 */
@Controller
public class AdvanceHostingPay extends HttpServlet {
	...
	@RequestMapping(value = "AdvanceHostingPay")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("ahpBean") AdvanceHostingPayBean ahpBean,
		...
		if (!ahpBean.getOut_advance_no().isEmpty()) {
			inputString
					.append(",out_advance_no=" + ahpBean.getOut_advance_no());
		}
		if (!ahpBean.getTicket().isEmpty()) {
			inputString.append(",ticket=" + ahpBean.getTicket());
		}
		if (!ahpBean.getValidate_code().isEmpty()) {
			inputString.append(",validate_code=" + ahpBean.getValidate_code());
		}
		if (!ahpBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + ahpBean.getExtend_param());
		}
		if (!ahpBean.getUser_ip().isEmpty()) {
			inputString.append(",user_ip=" + ahpBean.getUser_ip());
		}
		...
		ahpBean.setEncodeString(sortString);
		if (!masBaseBean.getNotify_url().isEmpty()) {
			ahpBean.setEncodeString(tool.removeFromString(
					ahpBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = ahpBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
