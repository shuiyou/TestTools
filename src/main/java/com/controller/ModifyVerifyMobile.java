...
import com.mgs.bean.ModifyVerifyMobileBean;
 ...
 */
@Controller
public class ModifyVerifyMobile extends HttpServlet {
	...
	@RequestMapping(value = "ModifyVerifyMobile")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("mvmBean") ModifyVerifyMobileBean mvmBean,
		...
		if (!mvmBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + mvmBean.getIdentity_id());
		}
		if (!mvmBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + mvmBean.getIdentity_type());
		}
		...
		if (!mvmBean.getClient_ip().isEmpty() && mvmBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + mvmBean.getClient_ip().replace(",", "#"));
		}else if (!mvmBean.getClient_ip().isEmpty() && !mvmBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + mvmBean.getClient_ip());
		}
		if (!mvmBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + mvmBean.getExtend_param());
		}
		...
		mvmBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			mvmBean.setEncodeString(tool.removeFromString(
					mvmBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = mvmBean.getEncodeString() + "&sign="
