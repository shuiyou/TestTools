...
import com.mgs.bean.ModifyWithholdAuthorityBean;
 ...
 */
@Controller
public class ModifyWithholdAuthority extends HttpServlet {
	...
	@RequestMapping(value = "ModifyWithholdAuthority")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("mwaBean") ModifyWithholdAuthorityBean mwaBean,
		...
		if (!mwaBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + mwaBean.getIdentity_id());
		}
		if (!mwaBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + mwaBean.getIdentity_type());
		}
		if (!mwaBean.getQuota().isEmpty() && mwaBean.getQuota().contains(",") ) {
			inputString.append(",quota=" + mwaBean.getQuota().replace(",", "#"));
		}else if(!mwaBean.getQuota().isEmpty()&& !mwaBean.getQuota().contains(",")){
			inputString.append(",quota=" + mwaBean.getQuota());
		}
		if (!mwaBean.getDay_quota().isEmpty() && mwaBean.getDay_quota().contains(",") ) {
			inputString.append(",day_quota=" + mwaBean.getDay_quota().replace(",", "#"));
		}else if(!mwaBean.getDay_quota().isEmpty()&& !mwaBean.getDay_quota().contains(",")){
			inputString.append(",day_quota=" + mwaBean.getDay_quota());
		}
		...
		if (!mwaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + mwaBean.getExtend_param());
		}
		...
		mwaBean.setEncodeString(sortString);
		if (!mwaBean.getQuota().isEmpty()) {
			mwaBean.setEncodeString(tool.removeFromString(
					mwaBean.getEncodeString(), "&", "&", "startswith",
					"quota")
					+ "&quota="
					+ tool.textEncode(mwaBean.getQuota(), "UTF-8"));
		}
		if (!mwaBean.getDay_quota().isEmpty()) {
			mwaBean.setEncodeString(tool.removeFromString(
					mwaBean.getEncodeString(), "&", "&", "startswith",
					"day_quota")
					+ "&day_quota="
					+ tool.textEncode(mwaBean.getDay_quota(), "UTF-8"));
		}
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			mwaBean.setEncodeString(tool.removeFromString(
					mwaBean.getEncodeString(), "&", "&", "startswith",
		...
		String requestString = mwaBean.getEncodeString() + "&sign="
