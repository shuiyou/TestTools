...
import com.mas.bean.P2PSpecialDepositBean;
 ...
 */
@Controller
public class p2pSpecialDeposit extends HttpServlet {
	...
	@RequestMapping(value = "p2pSpecialDeposit")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("phtBean") P2PSpecialDepositBean phtBean,
		...
		if (!phtBean.getAccount_identity().isEmpty()) {
			inputString.append(",account_identity=" + phtBean.getAccount_identity());
		}
		
		if (!phtBean.getAmount().isEmpty()) {
			inputString.append(",amount=" + phtBean.getAmount());
		}
		if (!phtBean.getSummary().isEmpty()) {
			inputString.append(",summary=" + phtBean.getSummary());
		}
		String encryptBankAccountNo = null;
		if (!phtBean.getBank_account_no().isEmpty()) {
			try {
				encryptBankAccountNo = rsa.encrypt(phtBean.getBank_account_no(),
						masBaseBean.getEncryptkey());
