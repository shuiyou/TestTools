...
import com.pas.bean.OneToOneFaceCompareNewEncryptBean;
 ...
 */
@Controller
public class OneToOneFaceCompareNewEncrypt extends HttpServlet
{
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "OneToOneFaceCompareNewEncrypt")
	public void doPost(@ModelAttribute("pasBaseBean") PasBaseBean pasBaseBean,
			@ModelAttribute("oneBean") OneToOneFaceCompareNewEncryptBean oneBean,
			...
			throws IOException, ServletException
	{
		...
		if (!oneBean.getOut_request_no().isEmpty()) {
			formparams.add(new OrderBasicNameValuePair("out_request_no", oneBean.getOut_request_no()));
		}

		if (!oneBean.getExtend_param().isEmpty()) {
			formparams.add(new OrderBasicNameValuePair("extend_param" , oneBean.getExtend_param()));
		}
		if(!oneBean.getPic_file_first().isEmpty()){
			formparams.add( new OrderBasicNameValuePair( "pic_file_first",oneBean.getPic_file_first()));
		}
		if(!oneBean.getPic_file_first2().isEmpty()){
			formparams.add( new OrderBasicNameValuePair( "pic_file_first2",oneBean.getPic_file_first2()));
		}
		if(!oneBean.getPic_file_second().isEmpty()){
			formparams.add( new OrderBasicNameValuePair( "pic_file_second",oneBean.getPic_file_second()));
		}
		if(!oneBean.getPic_file_second2().isEmpty()){
			formparams.add( new OrderBasicNameValuePair( "pic_file_second2",oneBean.getPic_file_second2()));
		}
