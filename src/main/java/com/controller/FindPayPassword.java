...
import com.mgs.bean.FindPayPasswordBean;
 ...
 */
@Controller
public class FindPayPassword extends HttpServlet {
	...
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("camBean") FindPayPasswordBean fppBean,
		...
		if (!fppBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + fppBean.getIdentity_id());
		}
		if (!fppBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + fppBean.getIdentity_type());
		}
		...
		if (!fppBean.getClient_ip().isEmpty() && fppBean.getClient_ip().contains(",")) {
			inputString.append(",client_ip=" + fppBean.getClient_ip().replace(",", "#"));
		}else if (!fppBean.getClient_ip().isEmpty() && !fppBean.getClient_ip().contains(",")){
			inputString.append(",client_ip=" + fppBean.getClient_ip());
		}
		if (!fppBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + fppBean.getExtend_param());
		}
		...
		fppBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			fppBean.setEncodeString(tool.removeFromString(
					fppBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = fppBean.getEncodeString() + "&sign="
