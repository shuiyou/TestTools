...
import com.mgs.bean.RelieveWithholdAuthorityBean;
 ...
 */
@Controller
public class RelieveWithholdAuthority extends HttpServlet {
	...
	@RequestMapping(value = "RelieveWithholdAuthority")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("rwaBean") RelieveWithholdAuthorityBean rwaBean,
		...
		if (!rwaBean.getIdentity_id().isEmpty()) {
			inputString.append(",identity_id=" + rwaBean.getIdentity_id());
		}
		if (!rwaBean.getIdentity_type().isEmpty()) {
			inputString.append(",identity_type=" + rwaBean.getIdentity_type());
		}
		...
		if (!rwaBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + rwaBean.getExtend_param());
		}
		...
		rwaBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			rwaBean.setEncodeString(tool.removeFromString(
					rwaBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = rwaBean.getEncodeString() + "&sign="
