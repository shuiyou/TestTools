...
import com.mas.bean.FundQueryPurchaseTradeBean;
import com.mas.bean.MasBaseBean;
@Controller
public class FundQueryPurchaseTrade extends HttpServlet {
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "FundQueryPurchaseTrade")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("fqptBean") FundQueryPurchaseTradeBean fqptBean,
		...
		if (!masBaseBean.getNotify_url().isEmpty()) {
			fqptBean.setEncodeString(tool.removeFromString(
					fqptBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = fqptBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
