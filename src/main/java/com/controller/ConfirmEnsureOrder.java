...
import com.b2c.bean.ConfirmEnsureOrderBean;
 ...
 */
@Controller
public class ConfirmEnsureOrder extends HttpServlet {
	...
	@RequestMapping(value = "ConfirmEnsureOrder")
	public void doPost(@ModelAttribute("b2cBaseBean") B2CBaseBean b2cBaseBean,
			@ModelAttribute("cfeoBean") ConfirmEnsureOrderBean cfeoBean,
		...
		if (!cfeoBean.getOut_request_no().isEmpty()) {
			inputString.append(",out_request_no=" + cfeoBean.getOut_request_no());
		}
		if (!cfeoBean.getOut_trade_no().isEmpty()) {
			inputString.append(",out_trade_no=" + cfeoBean.getOut_trade_no());
		}
		if (!cfeoBean.getSplit_list().isEmpty()) {
			inputString.append(",split_list=" + cfeoBean.getSplit_list());
		}
		if (!cfeoBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + cfeoBean.getExtend_param());
		}
		...
		cfeoBean.setEncodeString(sortString);
		if (!b2cBaseBean.getNotify_url().isEmpty()) {
			cfeoBean.setEncodeString(tool.removeFromString(
					cfeoBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		if (!cfeoBean.getSplit_list().isEmpty()) {
			cfeoBean.setEncodeString(tool.removeFromString(
					...
					"split_list")
					+ "&split_list="
					+ tool.textEncode(cfeoBean.getSplit_list(), "UTF-8"));
		}
		if (!cfeoBean.getExtend_param().isEmpty()) {
			cfeoBean.setEncodeString(tool.removeFromString(
					cfeoBean.getEncodeString(), "&", "&", "startswith",
					"extend_param")
					+ "&extend_param="
					+ tool.textEncode(cfeoBean.getExtend_param(), "UTF-8"));
		}

		String requestString = cfeoBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
