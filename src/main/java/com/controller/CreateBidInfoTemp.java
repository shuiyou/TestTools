...
import com.mas.bean.CreateBidInfoBean;
 ...
 */
@Controller
public class CreateBidInfoTemp extends HttpServlet {
    ...
    @RequestMapping(value = "CreateBidInfoTemp")
    public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
                       @ModelAttribute("cbiBean") CreateBidInfoBean cbiBean,
        ...
        extend_param = cbiBean.getBorrower_info_list().split("~");

        if (cbiBean.getExtend_param().contains("isRsaEncrypt^Y")) {
            extend_param[4] = rsa.encrypt(extend_param[4], masBaseBean.getEncryptkey());//手机号
            extend_param[5] = rsa.encrypt(extend_param[5], masBaseBean.getEncryptkey());//固定电话
            extend_param[12] = rsa.encrypt(extend_param[12], masBaseBean.getEncryptkey());//邮箱

            StringBuilder extend_param_temp = new StringBuilder("");
            for (String value : extend_param) {
                extend_param_temp.append(value + '~');
            }

            inputString.append(",borrower_info_list=" + extend_param_temp.toString().substring(0, extend_param_temp.length() - 1)); //已经加密好的extend_param  包含在加签字符串里

            extend_param[4] = tool.textEncode(extend_param[4], "UTF-8");//encode后的去替换
            extend_param[5] = tool.textEncode(extend_param[5], "UTF-8");
            extend_param[12] = tool.textEncode(extend_param[12], "UTF-8");
        ...
        if (cbiBean.getExtend_param().contains("isRsaEncrypt^Y")) {
            StringBuilder extend_param_temps = new StringBuilder("");
            for (String value : extend_param) {
                extend_param_temps.append(value + '~');
            }
            ...
            cbiBean.setEncodeString(tool.removeFromString(
                    ...
                    "borrower_info_list")
                    + "&borrower_info_list="
                    + extend_param_temps.toString().substring(0,extend_param_temps.length() - 1));
        }
