...
import com.mas.bean.FundCreatePurchaseRefundBean;
import com.mas.bean.MasBaseBean;
@Controller
public class FundCreatePurchaseRefund extends HttpServlet {
	private static final long serialVersionUID = 1L;

	@RequestMapping(value = "FundCreatePurchaseRefund")
	public void doPost(@ModelAttribute("masBaseBean") MasBaseBean masBaseBean,
			@ModelAttribute("fcprBean") FundCreatePurchaseRefundBean fcprBean,
		...
		if (!fcprBean.getOut_trade_no().isEmpty()) {
			inputString.append(",out_trade_no=" + fcprBean.getOut_trade_no());
		}
		if (!fcprBean.getOrig_outer_trade_no().isEmpty()) {
			inputString.append(",orig_outer_trade_no=" + fcprBean.getOrig_outer_trade_no());
		}
		...
		if (!fcprBean.getPayer_ip().isEmpty()) {
			inputString.append(",user_ip=" + fcprBean.getPayer_ip());
		}
		...
		if (!masBaseBean.getNotify_url().isEmpty()) {
			fcprBean.setEncodeString(tool.removeFromString(
					fcprBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = fcprBean.getEncodeString().replaceAll(
				"replaceFlag", ",")
