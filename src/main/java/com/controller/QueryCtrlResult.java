...
import com.mgs.bean.QueryCtrlResultBean;
 ...
 */
@Controller
public class QueryCtrlResult extends HttpServlet {
	...
	@RequestMapping(value = "QueryCtrlResult")
	public void doPost(@ModelAttribute("mgsBaseBean") MgsBaseBean mgsBaseBean,
			@ModelAttribute("qcrBean") QueryCtrlResultBean qcrBean,
		...
		if (!qcrBean.getOut_ctrl_no().isEmpty()) {
			inputString.append(",out_ctrl_no=" + qcrBean.getOut_ctrl_no());
		}
		if (!qcrBean.getExtend_param().isEmpty()) {
			inputString.append(",extend_param=" + qcrBean.getExtend_param());
		}
		...
		qcrBean.setEncodeString(sortString);
		if (!mgsBaseBean.getNotify_url().isEmpty()) {
			qcrBean.setEncodeString(tool.removeFromString(
					qcrBean.getEncodeString(), "&", "&", "startswith",
					"notify_url")
					+ "&notify_url="
		...
		String requestString = qcrBean.getEncodeString() + "&sign="
