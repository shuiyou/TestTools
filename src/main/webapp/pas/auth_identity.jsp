...
function show(){
	...
	switch(select.value){
		case 'func':
			...
			break;
		case 'test':
			address.value='https://testgate.pay.sina.com.cn/pas/gateway.do';
			partnerId.value='200003670082';
			break;
		case 'real':
			address.value='https://gate.pay.sina.com.cn/pas/gateway.do';
...
function showCerttype() {
	var select = document.getElementById('selectCertType');
	var certty = document.getElementById('cert_type');
	switch (select.value) {
	case 'IC':
		certty.value = 'IC';
		break;
...
function showAuthtype() {
	var select = document.getElementById('selectAuthType');
	var authty = document.getElementById('auth_type');
	
	switch (select.value) {
	case 'SMS_AUTH':
		authty.value='SMS_AUTH';
		break;
	case 'HISTORY_TRANS':
		authty.value='HISTORY_TRANS';
		break;
	default:
		authty.value = '';
		break;
...
function checkCertType() {
	if (document.getElementById('cert_type').value == "") {
		alert("请选择证件类型!");
		document.getElementById('selectCertType').focus();
		return false;
	}
...
function checkAuthType() {
	if (document.getElementById('auth_type').value == "") {
		alert("请选择证件类型!");
		document.getElementById('selectAuthType').focus();
		return false;
	}
...
<body>
  <div style="position: relative;margin-top: 50PX;margin-bottom: 50PX" class="container">
  	<div id="createMember">
  	  <div class="h4" >身份授权</div>
  	  	<div style="width: 800px">
  	  	<div>
  	      <form target="_blank" class="form-horizontal" role="form" name="login" action="../AuthIdentityPas" method="post" accept-charset="utf-8" onsubmit="return checkUrl();">
			<table class="table table-striped">
				...
				<tbody>
					...
					<tr>
						...
						<td>
    						<div class="col-sm-10">
    							<input type="text" name="service" class="form-control" id="service"  placeholder="接口名称" value="auth_identity">
    						</div>					
						</td>
						<td></td>
					</tr>
					...
					<tr>
						...
						<td>
    						<div class="col-sm-10">
    							<input type="text" name="account_name" class="form-control" id="account_name" placeholder="持卡人姓名" value="">
    						</div>					
						</td>
						<td></td>
					</tr>
					...
					<tr>
						...
						<td>
						    <div class="col-sm-10">
								<select id="selectCertType" class="form-control" onchange="showCerttype();">
												<option>请选择</option>
												<option value="IC">身份证</option>
												<option value="PP">护照</option>
												<option value="HMP">港澳通行证</option>
								</select>
							</div>
						</td>
					</tr>
					...
					<tr>
						<td><label class="col-sm-2 control-label">auth_type: </label></td>
						<td>
    						<div class="col-sm-10">
    							<input type="text" name="auth_type" class="form-control" id="auth_type" placeholder="授权认证类型" value="">
    						</div>					
						</td>
						<td>
						    <div class="col-sm-10">
								<select id="selectAuthType" class="form-control" onchange="showAuthtype();">
												<option>请选择</option>
												<option value="SMS_AUTH">短信验证</option>
												<option value="HISTORY_TRANS">历史交易</option>
								</select>
							</div>
						</td>
					</tr>
